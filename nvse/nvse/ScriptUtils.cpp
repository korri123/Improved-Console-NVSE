...
VariableInfo* GetVariableByName(Script::VarInfoList* entry, const char* varName)
{
	...
	{
		if (node->data && !_stricmp(node->data->name.m_data, varName))
			return node->data;
	}
	...
	do
	{
		if (_stricmp(node->Data()->name.CStr(), varName) == 0)
		{
			return node->Data();
		}
	...
	while (lines.NextToken(lineText) != -1)
	{
		...
		if (tokens.NextToken(token) != -1)
		{
			if (!_stricmp(token.c_str(), "begin"))
			{
				...
				{
					VariableInfo* varInfo = GetVariableByName(varList, token.c_str());
					if (!varInfo)
						return false;
	...
	while (lines.NextToken(lineText) != -1)
	{
		...
		if (tokens.NextToken(token) != -1)
		{
			...
			else if (!_stricmp(token.c_str(), "array_var"))
			{
				...
				VariableInfo* varInfo = GetVariableByName(&m_scriptBuf->vars, token.c_str());
	...
	if (refVar)
	{
		if (dotPos == -1)
		{
			if (refVar->varIdx)			// it's a variable
				return ScriptToken::Create(GetVariableByName(&m_scriptBuf->vars, refVar->name.m_data), 0, Script::eVarType_Ref);
...
VariableInfo* ExpressionParser::LookupVariable(const char* varName, Script::RefVariable* refVar)
{
	...
	return GetVariableByName(vars, varName);
}
						...
						*out = form;
						...
						switch (info->typeID) {
							...
							case kParamType_NonFormList:
								{
									if (form->Unk_3A() && (form->typeID != kFormType_BGSListForm)) {
										TESForm** out = va_arg(varArgs, TESForm**);
										*out = form;
									}
									else {
