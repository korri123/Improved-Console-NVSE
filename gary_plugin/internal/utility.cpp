...
bool DebugLog::OpenWrite(const char* filePath)
{
	fileName = filePath;
	if (fStream.OpenWrite(fileName))
	{
		fStream.Close();
		return true;
	}
...
void DebugLog::Message(char *msgStr)
{
	if (!fStream.OpenWrite(fileName)) return;
	if (indent)
		fStream.WriteBuf(kIndentLevelStr, indent);
	UInt32 length = StrLen(msgStr);
	msgStr[length] = '\n';
	fStream.WriteBuf(msgStr, length + 1);
	fStream.Close();
}
...
void PrintLog(const char *fmt, ...)
{
	...
	ImprovedConsole::s_log.Message(buffer);
	va_end(args);
}